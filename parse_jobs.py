...
def parse_jobs(csv_path: str):
    ...
    ndf = pd.DataFrame(nd)
    if "job_id" not in ndf:       ndf["job_id"] = ndf.index.astype(str)
    if "title" not in ndf:        ndf["title"] = "Unknown"
    if "company" not in ndf:      ndf["company"] = "Unknown"
